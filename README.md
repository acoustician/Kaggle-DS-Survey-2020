...
This notebook is an exploratory data analysis (EDA) of the most comprehensive dataset available on the view of machine learning and data science today 2020 [Kaggle Machine Learning & Data Science Survey](https://www.kaggle.com/c/kaggle-survey-2020/data). The survey answers covered demographic, education, employment, and technology usage to learn more about data science practitioners in 2020.
 ...
 # Overview 
  ...
  Kaggle platform conducted an industry survey every year with the intent to present a genuinely comprehensive view of the current insights of data science and machine learning. The questionnaire collected 20,036 data from the survey.
  
  There are 39 main [Questions](https://drive.google.com/file/d/1FXmCuhZxkiEZb1_DG_1dlRHg3vJxIXvW/view?usp=sharing) and 16 supplementary questions. Each questions has number of choices, In some question, there are one or more choices can select and in some questions only one option can choose 
...
The plots below give a general idea of the responses to the survey. The subsections that follow take a deeper dive, aiming to extract additional insights by combining the data available.
  ...
  This analysis focused on top most countries (3rd Question) which have maximum number of Data scientist.There are two bar plots first is for overall and second one is experience-wise.  
  There are lots of year range option in experience column which makes visualization little confusing therefore the Experience of the candidates are divided in three categories,   First is Professional (10+ year experience) second is Intermediate(3 to 10 year experience) and last one is Beginner(0-3 year of experience).  
...
The surveyor asked to the candidate that which programming language they used in a regular basis. Based on these question an analysis was made for the most used programming languages. Here also, candidate can select more then one choices for the particular question. That's why data are present in the different columns. Therefore, for better visualization a dictionary was defined and make a DataFrame from the dictionary in which all options and its value count are stored.
...
Candidates have multiple choices to select the one or more than one IDE. This analysis tells us about the popular IDE for data science.

Because of multiple choice, Data are present in different number of column. For better visualization, a dictionary is defined for combining the all columns and count them in a single DataFrame.
...
Surveyor asked to the candidate that which ML algorithm used on a regular basis. Here also, candidate can select more than one choices for the particular question. That's why data are present in the different columns. Therfore, for better visualization a dictionary was defined and make a DataFrame from the dictionary in which all options and its value count are stored.
...
Surveyor asked to the candidate that which BI tool used on a regular basis.Here also, candidate can select more than one choices for the particular question.That's why data are present in the different columns. Therefore, for better visualization a dictionary was defined and make a DataFrame from the dictionary in which all options and its value count are stored.
...
Candidate who participate in the survey are from different age group.This analysis focused on the candidate who take part the most in survey is from which age group.
...
# 9.Participate by diffrent profession people in the survey.
 The survey is conducted to get insights of the today's world in the Data Science field. In this survey people are from different profession are participated. Therefore, this analysis is made for get idea about which age group people participated most in the survey.
...
The EDA focused on understanding the Kaggle and Data Science Community. While spread worldwide, India and USA candidate are competitive and top in position. But, India leads in Basic and intermediate experienced people while USA leads in Professional people (people have more than 10 year of experience).
...
There are lots of machine learning algorithm used for regression and classification challenges. But, Logistic regression and Random Forest is the algorithms which is mostly used for machine learning.

NLP a branch of AI used different methods to train a model. Data scientist mostly uses Word embeddings/vectors (GLoVe, fastText, word2vec) methods to train the model.
Coming to the Business intelligence tools which gets the visualized analysis of data to solve the challenges of business Data scientist used Tableau and PowerBI mostly.

According to this analysis, if somebody wants to showcase your model and analysis to the world than Github is the best platform for you, Kaggle got third place in this analysis. Because, second most majority of people do not share their project publicly.
